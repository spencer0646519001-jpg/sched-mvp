...
def pick_chefs_for_day(
    ...
    candidates = [
        name for name in all_chefs if days_worked.get(name, 0) < max_days_per_week
    ]
    ...
    if len(picked) < min_chefs:
        ...
        overflow_candidates = sorted(
            all_chefs, key=lambda name: days_worked.get(name, 0)
        )
...
def choose_shift_for_person(
    ...
    # 從允許的班別中，挑使用次數最少的那個
    best_shift = min(allowed_shifts, key=lambda s: shift_count.get(s, 0))
