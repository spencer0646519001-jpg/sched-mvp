...
# -------- 班別與時數 --------
def build_shift_maps(shifts_list: List[dict]) -> Tuple[Dict[str, dict], Dict[str, float]]:
    """
...
def enforce_morning_requirements(assignments: dict, rules: dict):
    ...
    for station, required_count in station_require.items():
        ...
        # 計算目前有多少早班
        current_morning = [
            p for p in assigned_list if p["shift"] in morning_shifts
        ]
        ...
        # 不足 → 找出 非早班 的人
        non_morning = [
            p for p in assigned_list if p["shift"] not in morning_shifts
        ]
        if not non_morning:
            continue
...
# -------- 資格檢查（硬約束）--------
def eligible_today(person: dict, date_obj: datetime, shift_code: Optional[str], rules: dict,shifts_map: dict) -> bool:
...
# -------- 評分（軟約束）--------
def assignment_cost(person: dict, station: str, shift_code: str, rules: dict, date_obj: datetime) -> tuple[float, dict]:
    """
    回傳 (cost, extra_notes)
    成本越小越好：
...
def greedy_assign(date_str: str, absent: List[str]) -> dict:
    ...
    min_staff = int(rules["min_staff_weekend"] if is_holi else rules["min_staff_weekday"])
    max_staff = int(rules.get("max_staff_per_day", 9))

    # 當日站位需求
    station_need: Dict[str, int] = rules.get("stations", {})
    stations = list(station_need.keys())
    ...
    for p in people:
        if p.get("role") == "chef":
            if p["name"] not in absent and eligible_today(p, day, None, rules, shifts_map):
    ...
    while need_more() and headcount_total() < max_staff:
        ...
        for s in deficit_sorted:
            ...
            if len(assignments[s]) >= station_need[s] and headcount_total() >= min_staff:
                continue
            ...
            for p in employees:
                ...
                if not shift_code or not eligible_today(p, day, shift_code, rules, shifts_map):
                    continue
            ...
            hours_estimate[person["name"]] = hours_estimate.get(person["name"], 0.0) + \
...
def apply_manual_patch(
    ...
    for s, recs in assignments.items():
        for rec in recs:
            sc = rec["shift"]
            hours_estimate[rec["name"]] = hours_estimate.get(rec["name"], 0.0) + \
...
if __name__ == "__main__":
    ...
    ap.add_argument("--absent", help="臨時請假名單，以逗號分隔，如 'Chung,Masuda'", default=None)
    args = ap.parse_args()
